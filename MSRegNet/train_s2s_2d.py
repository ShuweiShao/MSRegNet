...
def train(data_dir,
    ...
    image = cv2.imread('imagesize.png', 0)
    # image = cv2.resize(image, (640, 1024), interpolation=cv2.INTER_CUBIC)
    image = image[0:1024, :]
    vol_size = image[np.newaxis, ..., np.newaxis].shape[1:-1]
    # prepare data files
    # for the CVPR and MICCAI papers, we have data arranged in train/validate/test folders
    # inside each folder is a /vols/ and a /asegs/ folder with the volumes
    # and segmentations. All of our papers use npz formated data.
    train_vol_names = glob.glob(os.path.join(data_dir, '*.png'))
    train_vol_names.sort()   # sequence
    ...
    data_gen = datagenerators_2d.example_gen(train_vol_names, batch_size=batch_size)
    # atlas_vol_bs = np.repeat(atlas_vol, batch_size, axis=0)
    # cvpr2018_gen = datagenerators.cvpr2018_gen(train_example_gen, atlas_vol_bs, batch_size=batch_size)
    # cvpr2018_gen_s2s = datagenerators.cvpr2018_gen_s2s(train_example_gen, batch_size=batch_size)
    # prepare callbacks
    ...
    with tf.device(gpu):
        ...
        mg_model.fit_generator(data_gen,
                               ...
                               epochs=nb_epochs,
                               callbacks=[save_callback, tb_config],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int,
                        ...
                        help="batch_size")
    # parser.add_argumroroent("--load_model_file", type=str,default='../models/cvpr2018_vm2_l2.h5',
