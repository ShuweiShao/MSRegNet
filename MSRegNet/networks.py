...
from keras.layers import Conv3D, Activation, Input, UpSampling3D, concatenate,Concatenate
...
def unet_core(vol_size, enc_nf, dec_nf, full_size=True, src=None, tgt=None, src_feats=1, tgt_feats=1):
    ...
    for i in range(len(enc_nf)):
        if i == 0:
            y = Conv(8, kernel_size=7, padding='same',
                         kernel_initializer='he_normal', strides=1)(x_enc[-1])
            y = MultiResBlock_1(y, enc_nf[i])
            y = ChannelSELayer(y, enc_nf[i])
            x_enc.append(y)
        else:
            y = MultiResBlock_1(x_enc[-1], enc_nf[i])
            y = ChannelSELayer(y, enc_nf[i])
            x_enc.append(y)

    # up-sample path (decoder)
    x = MultiResBlock_2(x_enc[-1], dec_nf[0])

    x = upsample_layer()(x)
    x = concatenate([x, x_enc[-2]])
    x = MultiResBlock_2(x, dec_nf[1])
    x = ChannelSELayer(x, dec_nf[1])

    x = upsample_layer()(x)
    x = concatenate([x, x_enc[-3]])
    x = MultiResBlock_2(x, dec_nf[2])
    x = ChannelSELayer(x, dec_nf[2])

    x = upsample_layer()(x)
    x = concatenate([x, x_enc[-4]])
    x = MultiResBlock_2(x, dec_nf[3])
    x = MultiResBlock_2(x, dec_nf[4])
    x = ChannelSELayer(x, dec_nf[4])

    x = upsample_layer()(x)
    x = concatenate([x, x_enc[0]])
    x = MultiResBlock_2(x, dec_nf[5])
    x = MultiResBlock_2(x, dec_nf[6])
    x = ChannelSELayer(x, dec_nf[6])
...
def cvpr2018_net_probatlas(vol_size, enc_nf, dec_nf, nb_labels,
    ...
    # warp atlas
    if warp_method == 'WARP':
        warped_atlas = nrn_layers.SpatialTransformer(interp_method='linear', indexing=indexing, name='warped_atlas')(
            [src_atl, flow])
    else:
...
def diff_net(vol_size, enc_nf, dec_nf, int_steps=7, src_feats=1,
    ...
    if bidir:
        y_tgt = nrn_layers.SpatialTransformer(interp_method='linear', indexing=indexing, name='warped_tgt')(
            [tgt, neg_flow])
...
def cond_img_atlas_diff_model(vol_shape, nf_enc, nf_dec,
    ...
    for i in range(extra_conv_layers):
        last_tensor = Conv(nb_conv_features, kernel_size=cond_conv_size, padding='same', name='atlas_ec_%d' % i)(
            last_tensor)
    ...
    if not templcondsi:
        ...
        cl = Conv(1, kernel_size=1, padding='same', use_bias=False, name='atlas_gen',
...
def img_atlas_diff_model(vol_shape, nf_enc, nf_dec,
    ...
    # pre-warp model (atlas model)
    pw = atl_img_model(vol_shape, mult=atl_mult, src=mn.inputs[0],
...
def conv_block_1(x_in, nf, strides=1):
    """
    specific convolution module including convolution followed by leakyrelu
    """
    ndims = 2
    ...
    Conv = getattr(KL, 'Conv%dD' % ndims)
    x_out = Conv(nf, kernel_size=3, padding='same',
                 kernel_initializer='he_normal', strides=strides, use_bias = False)(x_in)
...
def MultiResBlock_1(x_in, nf):

    Conv = getattr(KL, 'Conv%dD' % 2)
    #shortcut = AveragePooling2D((2, 2), strides=(2, 2))(x_in)
    shortcut = MaxPooling2D((2, 2), strides=(2, 2))(x_in)
    shortcut = Conv(nf, kernel_size=1, padding='same',
                 kernel_initializer='he_normal', strides=1)(shortcut)
    #shortcut = BatchNormalization()(shortcut)
    conv3x3 = conv_block(x_in, int(nf*0.5), 2)
    conv = concatenate([conv3x3, shortcut])
    conv5x5 = conv_block(conv, int(nf*0.5))
    out = concatenate([conv3x3, conv5x5])
    #out = BinarySpatialSELayer(shortcut, out)
...
def MultiResBlock_2(x_in, nf):

    Conv = getattr(KL, 'Conv%dD' % 2)
    shortcut = Conv(nf, kernel_size=1, padding='same',
                    kernel_initializer='he_normal', strides=1)(x_in)
    #shortcut = BatchNormalization()(shortcut)
    conv3x3 = conv_block(x_in, int(nf * 0.5))
    conv = concatenate([conv3x3, shortcut])
    conv5x5 = conv_block(conv, int(nf * 0.5))
    out = concatenate([conv3x3, conv5x5])
    #out = BinarySpatialSELayer(shortcut, out)
...
def ChannelSELayer(x, out_dim, ratio=16):
...
def SpatialSELayer(x, nf=1, strides=1):

    ndims = 2
    assert ndims in [1, 2, 3], "ndims should be one of 1, 2, or 3. found: %d" % ndims

    Conv = getattr(KL, 'Conv%dD' % ndims)
    squeeze = Conv(nf, kernel_size=1, padding='same',
                 kernel_initializer='he_normal', strides=strides)(x)
    excitation = activation(squeeze, 'sigmoid')
...
def BinarySpatialSELayer(x1, x2, nf=1, strides=1):
    ...
    Conv = getattr(KL, 'Conv%dD' % ndims)
    squeeze = Conv(nf, kernel_size=1, padding='same',
                   kernel_initializer='he_normal', strides=strides)(x1)
    excitation = activation(squeeze, 'sigmoid')

    scale = multiply([x2, excitation])

    return scale


def ChannelSpatialSELayer(x, out_dim):

   excitation_1 = ChannelSELayer(x, out_dim)
   excitation_2 = SpatialSELayer(x)
   scale = Add()([excitation_1, excitation_2])
